...
bool piece_is_in_final_position(uint index, enum color_t color, array_list_t *starting_position)
{
    ...
    for (size_t i = 0; i < array_list_length(starting_position); i++)
    {
...
{
    for (size_t i = 0; i < array_list_length(game->starting_position); i++)
    {
...
{
    uint lgt = 0;
    game_piece_t *tab = malloc(sizeof(game_piece_t) * array_list_length(game->captured_pieces_list));
    CHECK_MALLOC(malloc);
    for (uint i = 0; i < array_list_length(game->captured_pieces_list); i++)
    {
...
node_t *choose_random_move_for_piece(const game_t *game, uint piece)
{
    ...
    uint len_children = array_list_length(moves->children);
    ...
    len_children = array_list_length(move_ending->children);
...
bool check_complex_win(const game_t *game)
{
    ...
    {
        bool victory = true;
        for (uint i = 0; i < array_list_length(game->starting_position); i++)
        {
...
{
    if (array_list_length(node->children) == 0)
        return node;

    node_t *best_node = node;
    for (size_t i = 0; i < array_list_length(node->children); i++)
    {
